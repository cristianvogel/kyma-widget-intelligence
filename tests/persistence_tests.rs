...
fn test_kyma_widget_persistence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{} {}",
        "→".green(),
        format!("Using database path: {:?}", db_path).cyan()
    );
    ...
    println!(
        ...
        format!(
            "Stored {} widgets",
            stats.get("total_widgets").unwrap_or(&0)
        )
        .cyan()
    );
...
fn test_kyma_preset_persistence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        format!(
            "Stored {} presets",
            stats.get("presets_stored").unwrap_or(&0)
        )
        .cyan()
    );
    ...
    println!(
        "{} {}",
        "→".green(),
        "Testing preset persistence...".yellow()
    );
    system.flush()?;
    drop(system);
    ...
    println!(
        ...
        format!(
            "Reloaded {} presets",
            stats2.get("presets_stored").unwrap_or(&0)
        )
        .cyan()
    );
...
fn test_kyma_export_import() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{} {}",
        "→".green(),
        "Importing to new database...".yellow()
    );
    ...
    println!(
        ...
        "→".green(),
        format!(
            "Source: {} widgets, {} presets",
            stats1.get("total_widgets").unwrap_or(&0),
            stats1.get("presets_stored").unwrap_or(&0)
        )
        .cyan()
    );
...
fn test_kyma_suggestions_persistence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{} {}",
        "→".green(),
        "Testing persistence of suggestions...".yellow()
    );
    system.flush()?;
    drop(system);
    ...
    println!(
        "{} {}",
        "→".green(),
        format!("After reload: {} suggestions", suggestions2.len()).cyan()
    );
