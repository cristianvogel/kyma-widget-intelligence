...
fn main() {
    ...
    println!(
        "    Display Type Hash: {}",
        widget_record.features.display_type_hash
    );
...
fn create_sample_filtered_widget() -> FilteredWidgetDescription {
    ...
    filtered.insert(
        "label".to_string(),
        Value::String("Master Volume Control".to_string()),
    );
    filtered.insert(
        "minimum".to_string(),
    ...
    filtered.insert(
        "maximum".to_string(),
        Value::Number(Number::from_f64(127.0).unwrap()),
    );
    filtered.insert(
        "displayType".to_string(),
        Value::String("slider".to_string()),
    );
